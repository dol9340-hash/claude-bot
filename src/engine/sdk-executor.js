...
function formatSdkFailure(error) {
    const err = error;
    ...
    if (err?.exitCode !== undefined)
        details.push(`exitCode=${String(err.exitCode)}`);
    if (err?.signal)
...
export class SdkExecutor {
    ...
    async executeWithRetry(options) {
        ...
        for (let attempt = 0; attempt <= this.maxRetries; attempt++) {
            ...
            if (result.success)
                return result;
            const isTransient = result.errors.some(e => e.includes('timeout') || e.includes('ECONNRESET') || e.includes('rate_limit'));
    ...
    async execute(options) {
        ...
        if (config.maxTurnsPerTask)
            sdkOptions.maxTurns = config.maxTurnsPerTask;
        if (config.maxBudgetPerTaskUsd)
            sdkOptions.maxBudgetUsd = config.maxBudgetPerTaskUsd;
        if (config.model)
